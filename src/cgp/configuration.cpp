...
void cgp::configuration::rows(unsigned int rows) {
    _rows = rows;
}

void cgp::configuration::columns(unsigned int columns) {
    _columns = columns;
}
...
void cgp::configuration::runs(unsigned int runs) {
    _runs = runs;
}

void cgp::configuration::outputs(unsigned int outputs) {
    _outputs = outputs;
}
...
void cgp::configuration::mutationProbability(double mutationProbability) {
    if (_mutationProbability < 0) {
        throw std::invalid_argument(
            "Invalid argument for mutationProbability. Mutation probability must be superior to 0."
        );
    }
...
void cgp::configuration::minimumFitnessSolution(double minimumFitnessSolution) {
    if (_minimumFitnessSolution < 0) {
        throw std::invalid_argument(
            "Invalid argument for minimumFitnessSolution. Minimum fitness solution must be superior to 0."
        );
    }
...
void cgp::configuration::comparisonFitnessOperator(std::string comparisonFitnessOperator) {
...
void cgp::configuration::toString() const {
    ...
    std::cout << "_minimumFitnessSolution: " << _minimumFitnessSolution << std::endl;
    ...
    std::cout << "_comparisonFitnessOperator: " << _comparisonFitnessOperator << std::endl;
}
